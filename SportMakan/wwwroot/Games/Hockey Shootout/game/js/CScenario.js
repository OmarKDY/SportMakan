...
//        });
    ...
    this.createAreaGoal = function (oPos, oProperties, oColor, oInfo) {
        ...
        //this._createBoxDebug();

        return oGoalBodyLine;
    };

    this._createBoxDebug = function () {
        var oGoalShapeLine = new CANNON.Box(new CANNON.Vec3(BALL_RADIUS, 1, 1));

        var oGoalBodyLine = new CANNON.Body({mass: 1, material: _oWallMaterial});
        oGoalBodyLine.addShape(oGoalShapeLine);

        oGoalBodyLine.position.set(POSITION_BALL.x, POSITION_BALL.y, POSITION_BALL.z+2);
        oGoalBodyLine.collisionResponse = 1;

        _oWorld.addBody(oGoalBodyLine);

        if (SHOW_3D_RENDER) {
            var oMaterial = new THREE.MeshPhongMaterial({color: 0x111111, specular: 0x111111, shininess: 70});
            _oDemo.addVisual(oGoalBodyLine, oMaterial);
        }
    ...
    this._createBallBody = function () {
        _oBallShape = new CANNON.Cylinder(BALL_RADIUS, BALL_RADIUS, BALL_HEIGHT, BALL_SEGMENTS);
    ...
    this.addImpulse = function (oBody, oVec3) {
        var v3ContactPoint = new CANNON.Vec3(0, -BALL_RADIUS, 0);

        oBody.applyLocalImpulse(oVec3, v3ContactPoint);
    };
    ...
    this.addForce = function (oBody, oVec3) {
        ...
        oBody.applyForce(v3Force, oBody.position);
    };
