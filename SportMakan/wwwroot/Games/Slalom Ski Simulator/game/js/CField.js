function CField(iX,iY,oParentContainer){
    ...
    this._init = function(iX,iY){
        ...
        var oData = {
            ...
            frames: {width: 1600, height: oSpriteField.height},
            animations: {start: 0, anim: [0,29,"anim"],arrival:[30,86,"end"],end:86}
        };

        var oSpriteSheet = new createjs.SpriteSheet(oData);
        _oSprite = createSprite(oSpriteSheet,"start",0,0,1600,oSpriteField.height);
        ...
        for(var k=0;k<2;k++){
            var oSponsorLeft = new CSponsor(510,345,true,_oContainer);
            _aLeftSponsors.push(oSponsorLeft);
            
            var oSponsorRight = new CSponsor(1070,345,false,_oContainer);
        ...
        var oSpriteSheet = new createjs.SpriteSheet(oData);
        _oArrival = createSprite(oSpriteSheet,"start",0,0,1600,624);
        _oArrival.y = -iY-18;
    ...
    this._onAnimChange = function(evt){
        if(evt.currentTarget.currentAnimation === "arrival"){
            ...
            }else if(evt.currentTarget.currentFrame >= 81 && !_bEndTrack){
                _bEndTrack = true;
                s_oGame.endTrack();
            }
        }
