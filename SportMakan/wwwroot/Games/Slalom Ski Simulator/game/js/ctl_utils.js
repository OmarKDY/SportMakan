...
function sizeHandler() {
	...
	var destH = Math.round(CANVAS_HEIGHT * multiplier);
        ...
        if(s_oMenuSelect != null){
            s_oMenuSelect.refreshButtonPos( );
        }
...
function formatTime(iTime,bNoMins,iNumRealDigit){	
    
    iTime/=1000;
    var iMins = Math.floor(iTime/60);
    var iSecs = iTime-(iMins*60);
    iSecs = parseFloat(iSecs).toFixed(iNumRealDigit);
...
function playSfxSound(szSound,iVolume,bLoop){
...
function playMusicSound(szSound,iVolume,bLoop){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){

        s_aMusic[szSound].play();
        s_aMusic[szSound].volume(iVolume);

        s_aMusic[szSound].loop(bLoop);

        return s_aMusic[szSound];
    }
    return null;
}

function stopSfxSound(szSound){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        s_aSounds[szSound].stop();
    }
}   

function stopMusicound(szSound){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        s_aMusic[szSound].stop();
    }
}   

function setSfxVolume(szSound, iVolume){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        s_aSounds[szSound].volume(iVolume);
    }
}  

function setMusicVolume(szSound, iVolume){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        s_aMusic[szSound].volume(iVolume);
    }
}  

function setSfxMute(szSound, bMute){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        s_aSounds[szSound].mute(bMute);
    }
}

function setMusicMute(szSound, bMute){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        s_aMusic[szSound].mute(bMute);
    }
}
...
function isMusicPlaying(szSound){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        return s_aMusic[szSound].playing();
    }
    return false;
}

function muteAllSfx(bMute){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        for(var k in s_aSounds){
            s_aSounds[k].mute(bMute);
        }
    }
    
}

function muteAllMusic(bMute){
    if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
        for(var k in s_aMusic){
            s_aMusic[k].mute(bMute);
        }
    }
...
function fullscreenHandler(){
    ...
    if(s_oMenuSelect !== null){
        s_oMenuSelect.resetFullscreenBut();
    }
}
